...
const main = () => {
  const begin = getMintInfo(MINTING_DATE.market.begin);
  const end = getMintInfo(MINTING_DATE.market.end);
  const obj = { begin, end };
  const folderPath = `${BASE_PATH}/__test__/data`;
  makeDir(folderPath);
  writeJson(`${folderPath}/market-release-time${EXT}`, obj);
  console.log(`file saved: ${folderPath}/market-release-time${EXT}`);
};
